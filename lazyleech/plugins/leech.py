...
@Client.on_message(filters.command(['ddl', 'ddl@anu1bot', 'zipddl@anu1bot', 'zipddl', 'fileddl', 'fileddl@anu1bot']) & filters.chat(ALL_CHATS))
...
async def handle_leech(client, message, gid, reply, user_id, flags):
    ...
    while torrent_info['status'] in ('active', 'waiting', 'paused'):
        ...
        text = f'''<b>Downloading</b> • {html.escape(tor_name)}
{html.escape(return_progress_string(completed_length, total_length))}
<b>Total</b> • {formatted_total_length}
